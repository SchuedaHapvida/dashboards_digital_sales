...
interface Agente { nome: string; chats: number; vendas: number; conversao: number }
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-50">
            {sorted.map((a, i) => {
              ...
              const convColor =
                a.conversao >= 10 ? "text-emerald-600 font-bold" :
                a.conversao >= 6  ? "text-[#F47920] font-semibold" :
                "text-slate-400";
              return (
                <tr key={a.nome} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-2.5">
                    <div>
                      <p className="font-medium text-slate-700 text-xs leading-tight">{a.nome}</p>
                  ...
                  <td className="px-4 py-2.5 text-right text-slate-600">{a.chats.toLocaleString("pt-BR")}</td>
                  <td className="px-4 py-2.5 text-right font-semibold text-[#003087]">{a.vendas}</td>
                  <td className={`px-4 py-2.5 text-right ${convColor}`}>{a.conversao.toFixed(1)}%</td>
                </tr>
              );
            })}
          </tbody>
